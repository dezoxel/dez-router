define(['microevent'], function(MicroEvent) {
  ...
  Router.prototype.when = function(routePattern, routeDefinition) {
    this._addRoute(routePattern, routeDefinition);
  ...
  Router.prototype.defaultRoute = function(routeDefinition) {
    this.when('', routeDefinition);
  ...
  // TODO: Simplify
  Router.prototype._fetchRouteParams = function(url, routeDefinition) {
    var paramsMatcher = /^(?:&[^=]+=[^&]*)+/;
    var parts = url.match(routeDefinition.regex);
    ...
    parts.forEach(function(part, i) {
      if (part && part !== '&') {
        ...
        routeParams[routeDefinition.names[i]] = isNaN(parseFloat(value)) ? value : parseFloat(value);
      }
    });

    var resultParams = extend(routeDefinition, queryStringParams);
